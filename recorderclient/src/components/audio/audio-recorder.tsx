...
export default function AudioRecorder({ isAuthenticated = false, onResultsChange, initialResults = [] }: AudioRecorderProps) {
  ...
  const getTranscriptContent = (resultId: number) => {
    ...
    if (transcriptMap[resultId]) {
      console.log(`AudioRecorder - Using stored transcript for result ID ${resultId}:`, transcriptMap[resultId].substring(0, 50) + '...');
    ...
    if (result && result.type === 'transcribe' && result.content) {
      console.log(`AudioRecorder - Found transcript in processedResults for ID ${resultId}:`, result.content.substring(0, 50) + '...');
    ...
    // If not in map or processedResults, return the default content
    console.log(`AudioRecorder - No stored transcript found for result ID ${resultId}, using default`);
    return "This is a simulated transcript of your audio recording. It would contain all the spoken words detected in your recording. In a real implementation, this would be generated by a speech-to-text service.\n\nThe transcript would be formatted with paragraphs and punctuation to make it easy to read. It might also include timestamps or speaker identification depending on the service used.";
  };
  ...
  const toggleCardExpanded = (id: number) => {
    ...
    // Update the expanded state in processedResults
    ...
    // If we're expanding a card, ensure we have the transcript content
    const result = processedResults.find(r => r.id === id);
    if (result && !result.expanded && result.type === 'transcribe') {
                          ...
                          <div className="mt-4">
                              ...
                              onClick={(e) => {
                                ...
                                if (transcriptEl) {
                                  ...
                                  if (!transcriptEl.classList.contains('hidden')) {
                                    console.log(`AudioRecorder - Showing transcript for ID ${result.id}`);
                                    
                                    // If we don't have the content in the map, try to get it from the result
                                    if (!transcriptMap[result.id] && result.content) {
                                      console.log(`AudioRecorder - Adding missing transcript to map for ID ${result.id}`);
                                      setTranscriptMap(prevMap => ({
                                        ...prevMap,
                            ...
                            <div id={`transcript-${result.id}`} className="p-3 bg-white/5 rounded-lg hidden">
                              <div className="whitespace-pre-line text-white/80 text-xs">
                                {getTranscriptContent(result.id)}
                              </div>
                            </div>
                          </div>
                        )}
